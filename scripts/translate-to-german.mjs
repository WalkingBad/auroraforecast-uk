...
console.log(`📖 Found ${keys.length} keys to translate from Russian to German`);
...
async function translateAll() {
  ...
  for (let i = 0; i < keys.length; i++) {
    ...
    if (deText === ruText || !deText) {
      deData[key] = enData[key] || ruText;
      failed++;
    } else {
      deData[key] = deText;
      translated++;
  ...
  console.log(`\n✅ German translations written to: ${deFile}`);
  console.log(`📁 Total keys: ${Object.keys(deData).length}`);
}
