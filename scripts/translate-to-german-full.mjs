...
const deFile = path.join(__dirname, '../src/data/web-strings/de.json');
const enFile = path.join(__dirname, '../src/data/web-strings/en.json');

const ruData = JSON.parse(fs.readFileSync(ruFile, 'utf-8'));
const deData = JSON.parse(fs.readFileSync(deFile, 'utf-8'));
...
for (const key of keys) {
  if (deData[key] === enData[key]) {
    untranslatedKeys.push(key);
  }
}

console.log(`⚠️  Found ${untranslatedKeys.length} untranslated German keys to fix\n`);
...
function translateText(text, retries = 5) {
  return new Promise((resolve) => {
    ...
    const url = `https://api.mymemory.translated.net/get?q=${encodedText}&langpair=ru|de`;

    const attemptTranslate = (attempt = 0) => {
      if (attempt >= retries) {
        console.warn(`❌ Failed: ${text.substring(0, 30)}...`);
...
async function translateAll() {
  ...
  console.log('🔄 Translating German...\n');
  ...
  for (let i = 0; i < untranslatedKeys.length; i++) {
    ...
    const deText = await translateText(ruText);
    if (deText !== enData[key]) {
      deData[key] = deText;
      translated++;
    }
  ...
  fs.writeFileSync(deFile, JSON.stringify(deData, null, 2), 'utf-8');
  console.log(`✅ Written to: ${deFile}`);
}
